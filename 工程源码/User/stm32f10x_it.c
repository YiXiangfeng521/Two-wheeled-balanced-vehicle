...
/**********************************************************************
 * 函数名称： MPU6050_IRQHandler
 * 功能描述： 当陀螺仪与加速度计数据都准备好时，会执行一次该中断服务函数
              执行该函数的频率由MPU6050的采样频率决定
 * 输入参数： 无
 * 输出参数： 无
 * 返 回 值： 
 * 修改日期        版本号     修改人        修改内容
...
void MPU6050_IRQHandler(void)
{
	...
	if(EXTI_GetITStatus(MPU6050_INT_EXTI_LINE)!=RESET) //一级判定
	{
		
		EXTI_ClearITPendingBit(MPU6050_INT_EXTI_LINE); //清除中断标志位
		
		if(GPIO_ReadInputDataBit(MPU6050_INT_GPIO_PORT, MPU6050_INT_GPIO_PIN) == 0) //二级判定
		{
		   Set_Flags(1); //设置标志位为1
